...
- **Does the diff content match the plan's TYPE?** If TYPE is FEATURE or REDESIGN but the diff touches ZERO runtime files (only `*.md`, `.claude/**`, `.github/**`), the TYPE is misclassified — FAIL. If TYPE is DOCS but the diff touches any non-`.md` file, FAIL. If TYPE is PIPELINE-INFRA but the diff touches app runtime code (`apps/*/src/**`), FAIL.
